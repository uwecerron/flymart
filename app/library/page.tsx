...
export default function Library() {
  ...
  return <><header><Link className="brand" href="/"><span className="brand-icon"><Gamepad2/></span>fly<span>mart</span></Link><Button asChild variant="outline"><Link href="/">Back to marketplace</Link></Button></header><main><section className="library"><p className="eyebrow">BUYER LIBRARY</p><h1>Your strange little collection.</h1><p className="lede">Paid files stay private. Each button creates a download link that expires after one minute.</p>{message && <div className="notice">{message}</div>}<div className="library-list">{purchases.map((purchase) => <article key={purchase.listing_id}><div><strong>{purchase.listings?.title ?? "Creator release"}</strong><p>{purchase.listings?.description}</p></div><Button onClick={() => download(purchase.listing_id)}><Download size={16}/> Download ZIP</Button></article>)}</div></section></main></>;
}
