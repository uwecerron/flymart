...
export default function Admin() {
  ...
  return <><header><Link className="brand" href="/"><span className="brand-icon"><Gamepad2/></span>fly<span>mart</span></Link><Button asChild variant="outline"><Link href="/">Back to marketplace</Link></Button></header><main><section className="library"><p className="eyebrow">OWNER REVIEW</p><h1>Keep the catalog strange, not random.</h1><p className="lede">Inspect the demo or repository and the uploaded package before approval. Publishing also requires the seller’s verified Stripe payout account.</p>{message && <div className="notice">{message}</div>}<div className="library-list">{listings.map((listing) => <article key={listing.id}><div><span className="tag">{listing.status.toUpperCase()} · {listing.genre}</span><h2>{listing.title}</h2><p>{listing.description}</p><small>By {listing.creator_name}</small>{listing.repo_url && <p><a className="underline" href={listing.repo_url} target="_blank" rel="noreferrer">Inspect project URL ↗</a></p>}</div><div className="review-actions"><Button variant="outline" onClick={() => inspect(listing.id)}><Download/> Inspect ZIP</Button><Button disabled={listing.status === "published"} onClick={() => review(listing.id, "published")}><Check/> Approve</Button><Button variant="outline" disabled={listing.status === "rejected"} onClick={() => review(listing.id, "rejected")}><X/> Reject</Button></div></article>)}</div></section></main></>;
}
