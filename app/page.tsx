...
export default function Home() {
  ...
  return <>
    ...
    <main>
      <div className="arcade-marquee"><span>● ARCADE ONLINE</span><div>FLY BRAIN GAMES <b>✦</b> REAL SOURCE RUNTIMES <b>✦</b> TINY NEURONS, BIG SCORES</div><span>{String(listings.length).padStart(2, "0")} CABINETS</span></div>
      ...
      <section id="browse" className="catalog"><div className="section-heading"><h2>Choose your cabinet<span>{String(listings.length).padStart(2, "0")} / SOURCE-VERIFIED FLY-BRAIN EXPERIMENTS</span></h2><div className="search"><Search size={17}/><Input aria-label="Search games" placeholder="Search the arcade…" value={query} onChange={(event) => setQuery(event.target.value)}/></div></div><div className="filters"><div className="status-dot"/><strong>THE SWARM IS AWAKE</strong><span>Playable means the listed repository is running · Creator releases are $2.99</span></div><div className="game-grid">{visible.map((item, index) => <article key={item.id} className="game-card"><div className="cabinet-number">CABINET {String(index + 1).padStart(2, "0")}</div><button className="art-button" onClick={() => setSelected(item)}><Art listing={item}/><span className="play-overlay"><Play size={11} fill="currentColor"/> {runtimeLabel(item)}</span></button><div className="game-info"><div className="game-meta"><span>{item.genre}</span><span>{item.runtime_kind === "browser" ? "LIVE SOURCE" : item.status === "free" ? "SOURCE VERIFIED" : "CREATOR RELEASE"}</span></div><h3>{item.title}</h3><p>{item.description}</p><div className="creator"><Code2 size={14}/>{item.creator_name}</div><div className="card-bottom"><span>{item.runtime_kind === "browser" ? "FREE PLAY" : item.status === "free" ? "FREE SOURCE" : "$2.99 USD"}</span><Button variant="outline" onClick={() => setSelected(item)}>{runtimeLabel(item)} <ArrowUpRight size={14}/></Button></div></div></article>)}</div>{!visible.length && <div className="empty"><Box/><h3>No cabinets found</h3><p>Try another game or experiment.</p></div>}</section>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) { setSelected(null); setActiveRuntimeId(null); } }}><DialogContent className="game-dialog !w-[min(96vw,1180px)] !max-w-[1180px]"><DialogTitle>{selected?.title}</DialogTitle><DialogDescription>{selected?.status === "free" ? selected.runtime_kind === "browser" ? "Authentic upstream browser runtime · source and commit verified below." : "Verified upstream source · runtime requirements shown below." : "One-time purchase · $2.99 USD"}</DialogDescription>{selected?.status === "free" ? <div className="demo">{selected.runtime_kind === "browser" && selected.runtime_path ? activeRuntimeId === selected.id ? <iframe className="authentic-runtime" src={selected.runtime_path} title={`${selected.title} authentic upstream runtime`} allow="cross-origin-isolated"/> : <div className="runtime-gate"><BrainCircuit/><strong>139,255 neurons are ready</strong><p>The real connectome is a 12.4 MB one-time download. It runs locally in your browser and is cached for repeat plays.</p><Button onClick={() => setActiveRuntimeId(selected.id)}><Play fill="currentColor"/> Start actual simulation</Button></div> : <div className="runtime-unavailable"><BrainCircuit/><strong>Authentic runtime is not browser-hostable yet</strong><p>{selected.runtime_note}</p></div>}<p>{selected.description}</p><div className="provenance"><span>RUNNING CODE</span><strong>{selected.runtime_kind === "browser" ? "Pinned upstream source" : "No simulation substituted"}</strong>{selected.source_commit && <code>{selected.source_commit}</code>}<small>{selected.runtime_note}</small></div><div className="actions">{selected.download_path && <Button asChild><a href={selected.download_path} download>Download source ZIP</a></Button>}{selected.runtime_path && <Button asChild variant="outline"><a href={selected.runtime_path} target="_blank" rel="noreferrer">Open full screen ↗</a></Button>}{selected.repo_url && <Button asChild variant="outline"><a href={selected.repo_url} target="_blank" rel="noreferrer">Original GitHub ↗</a></Button>}</div></div> : selected && <><div className="checkout-summary"><Gamepad2/><div><strong>{selected.title}</strong><p>Permanent access to this creator release</p></div><strong>$2.99</strong></div><div className="notice"><strong>Secure card checkout</strong><p>Stripe processes the payment and routes the seller payout. Your purchase unlocks a short-lived private download link.</p></div><Button disabled={busy || !config.marketplaceReady} onClick={() => checkout(selected)}><CreditCard/> {busy ? "Starting checkout…" : "Buy with Stripe · $2.99"}</Button>{!config.marketplaceReady && <p className="fine">The owner must finish Stripe and Supabase setup before checkout opens.</p>}</>}</DialogContent></Dialog>
